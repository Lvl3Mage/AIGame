...
[GlobalClass]
public partial class ScreamStrategy : Node, IAgentTaskProvider, IAgentEventListener<ScreamEvent>
{
	...
	[Export] float recruitRadius = 500f;
	[Export] float taskExpiryTime = 15f;

	float accumulatedStrength = 0f;
	Queue<ScreamEvent> playerEventHistory =[];

	void AddEvent(ScreamEvent agentEvent)
	{
	...
	public void OnEvent(ScreamEvent agentEvent)
	{
		AddEvent(agentEvent);
	...
	void BuildTasks()
	{
		ScreamEvent playerEvent = playerEventHistory.Last();
		...
		var tasks = targets.Select(target => new AgentTask{
			...
			TaskRadius = recruitRadius,
			CreationTime = Time.GetTicksMsec() / 1000f,
			TaskPriority = AgentTask.Priority.Urgent
		});
		pendingTasks.Clear();
